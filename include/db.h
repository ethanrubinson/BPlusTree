...
class DB {
  ...
  public:
   ...
   ~DB();
	...
	//modified by Mingsheng Hong 06.10.22
	//unpin the header page of the B+ tree that manages database catalog.
	Status DestroyCatalogBTree();
  ...
  private:
	...
	BTreeFile *btree;
	HeapFile* heapFile;
	int SPACE_MAP_START; //it is 1 when we do not use btree to manage catalog,
						//but will be 2 if we do.
